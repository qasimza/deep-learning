# DL5C.py CS5173/6073 cheng 2023
# autoregression on hospitalization
# making a new module with Linear a submodule
# using MSELoss
# Goodfellow Algorithm 8.3 SGD with Nesterov momentum
# with random sample of training data
# according to PyTorch SGD implementation
# Usage:  python DL5C.py
...
for param in model.parameters():
    param.v = torch.zeros_like(param)
    param.c = torch.zeros_like(param)
...
for i in range(rounds):
    ...
    loss.backward()
    for param in model.parameters():
        param.v = alpha * param.v - eta * param.grad
        param.data = param.c + param.v
