...
for param in model.parameters():
    param.s = torch.zeros_like(param)
    param.r = torch.zeros_like(param)
rho1 = 0.9
rho2 = 0.999
delta = 1e-8
...
for i in range(rounds):
    ...
    loss = loss_fun(y_pred, y[batch_indices]) / batch_size
    losses[i] = loss.item()
    model.zero_grad()
    loss.backward()
    for param in model.parameters():
        param.s = # one line in Algorithm 8.7
        param.r = rho2 * param.r + (1 - rho2) * param.grad.square()
        param.data -= eta * ... # the next four lines in Algorithm 8.7
        rho1t *= rho1 # so rho1t is rho1^t, to be used above
        rho2t *= rho2 # and rho2t is rho2^t
