...
class GRUScratch(torch.nn.Module): 
    def __init__(self):
        ...
        self.resetgate = torch.nn.Linear(input_size + hidden_size, hidden_size)
        self.updategate = torch.nn.Linear(input_size + hidden_size, hidden_size)
    ...
    def forward(self, x):
        ...
        for X in X2:
            ...
            R = torch.sigmoid(self.resetgate(input))
            Z = torch.sigmoid(self.updategate(input))
            input2 = torch.cat((X, R * H), 1)
